...
function updateAgeFilterButtonState() {
  ...
  btn.textContent = isAgeFilterActive() ? "Kết quả đã lọc - Nhấn vào đây để thoát!" : "🎂 Lọc theo tuổi";
}
...
function renderColumnList(headers) {
  ...
  headers.forEach((col, index) => {
    ...
    // Khóa cột CCCD - không cho đổi tên
    if (col === "CCCD") {
      nameInput.disabled = true;
      nameInput.style.backgroundColor = "#f0f0f0";
      nameInput.style.color = "#666";
      nameInput.style.cursor = "not-allowed";
      nameInput.title = "Cột CCCD không thể đổi tên";
    }
...
function addNewColumn() {
  ...
  // Không cho phép thêm cột tên "CCCD" vì đã tồn tại
  if (name === "CCCD") {
    showMsg("sheetConfigMsg", `Cột "CCCD" đã tồn tại và không thể thêm lại.`, true);
    return;
  }
...
async function saveSheetConfig() {
  ...
  document.querySelectorAll(".col-name-input").forEach(input => {
    ...
    if (val) {
      ...
      if (val === "CCCD") cccdFound = true;
    }
  });
  
  // Đảm bảo luôn có cột CCCD
  if (!cccdFound) {
    showMsg("sheetConfigMsg", `Thiếu cột bắt buộc: "CCCD"`, true);
    return;
  }
...
function openExportModal() {
  ...
  const liveCols = displayHeaders.filter(f => f !== "MatKhau" && f !== "__rowIndex" && f !== "hasPassword" && f !== "STT");
  const displayCols = liveCols.length > 0 ? liveCols : ["Họ và tên", "Ngày sinh", "Ngày vào đảng", "Ngày chính thức",
    "Dân tộc", "Tôn giáo", "CCCD", "Thẻ đảng viên", "Học vấn (Vd: 10/10, 12/12...)",
    "Chuyên môn", "LLCT", "Số điện thoại", "Chi bộ cũ",
    "Miễn sinh hoạt", "Nơi thường trú"];
...
function doExportExcel() {
  ...
  data.sort((a, b) => {
    ...
    switch (sortField) {
      ...
      case "tuoi":
        valA = getAge(a["Ngày sinh"]);
        valB = getAge(b["Ngày sinh"]);
        // Đưa các bản ghi thiếu ngày sinh (không tính được tuổi) xuống cuối
        if (valA === null && valB === null) return 0;
        if (valA === null) return 1;
        if (valB === null) return -1;
        return sortOrder === "asc" ? valA - valB : valB - valA;
      case "tuoi_dang":
        valA = getPartyAge(a["Ngày vào đảng"]);
        valB = getPartyAge(b["Ngày vào đảng"]);
        // Đưa các bản ghi thiếu ngày vào đảng (không tính được tuổi đảng) xuống cuối
        if (valA === null && valB === null) return 0;
        if (valA === null) return 1;
        if (valB === null) return -1;
        return sortOrder === "asc" ? valA - valB : valB - valA;
